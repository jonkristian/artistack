...
<div>
  <div>
    <span class="mb-2 block text-sm text-gray-400"
      >Select images ({selectedIds.length} selected)</span
    >
    {#if media.length > 0}
      <div class="grid grid-cols-4 gap-2">
        {#each media as item (item.id)}
          <button
            ...
            class="relative overflow-hidden rounded-lg transition-all {selectedIds.includes(item.id)
    ...
    {:else}
      <p class="py-4 text-center text-sm text-gray-600">
        No media uploaded yet. Upload images in the media library first.
      </p>
    {/if}
  </div>
</div>
