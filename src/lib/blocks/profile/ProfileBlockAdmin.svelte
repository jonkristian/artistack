<script lang="ts">
  ...
  function detectSocialPlatform(url: string): string | null {
    try {
      ...
      if (hostname.includes('apple') && hostname.includes('music')) return 'apple_music';
      if (hostname.includes('threads')) return 'threads';
      if (hostname.includes('mastodon') || hostname.includes('mstdn')) return 'mastodon';
      if (hostname.includes('bluesky') || hostname.includes('bsky')) return 'bluesky';
      if (hostname.includes('linkedin')) return 'linkedin';
      if (hostname.includes('github')) return 'github';
      if (hostname.includes('gitlab')) return 'gitlab';
      if (hostname.includes('substack')) return 'substack';
      if (hostname.includes('linktr.ee') || hostname.includes('linktree')) return 'linktree';
      ...
      if (hostname.includes('ko-fi')) return 'kofi';
      if (hostname.includes('buymeacoffee') || hostname.includes('bmc')) return 'buymeacoffee';
...
<div class="space-y-4">
  ...
  <div class="border-t border-gray-800 pt-4">
    <span class="mb-2 block text-sm text-gray-400">Social Links</span>
    ...
    {#if socialLinks.length > 0}
      <div class="mb-3 flex flex-wrap gap-2">
        {#each socialLinks as link (link.id)}
          <div
            class="group relative flex items-center gap-1.5 rounded-lg bg-gray-800 px-2.5 py-1.5"
          >
    ...
    <p class="mt-1 text-xs text-gray-600">Any social or music platform URL</p>
  </div>
</div>
