<script lang="ts">
  ...
  /*
   * The site's shows, not this block's — a show belongs to the band, and this
   * is one place it gets displayed. Two pages can carry this block and both
   * stay right without anyone entering the tour twice.
   */
  const today = new Date().toISOString().split('T')[0];
  const upcomingShows = $derived(
    config.limit != null
      ? shows.filter((t) => t.date >= today).slice(0, config.limit)
      : shows.filter((t) => t.date >= today)
  );
  const pastShows = $derived(shows.filter((t) => t.date < today).reverse());
    ...
    <div class="space-y-2">
      {#each upcomingShows as tour (tour.id)}
        ...
        >
          ...
          <!-- Poster, when there is one. Portrait, the shape a flyer is. -->
          {#if tour.imageUrl}
            <div class="w-14 flex-shrink-0 self-center">
              <img
                src={tour.imageUrl}
                alt=""
                loading="lazy"
          ...
          <div class="min-w-0 flex-1">
            ...
            {#if tour.lineup && tour.lineup.length > 0}
              <!--
                Logos where an act has one, its name where it doesn't — the
                line-up reads the same either way, and an act without artwork
                doesn't leave a hole in the row.
              -->
              <div class="mt-1 flex flex-wrap items-baseline gap-x-3 gap-y-1">
                {#each tour.lineup as act (act.name)}
                  <!-- Baseline, not centre: the time is smaller than the name,
                       so centring the two sits it low against the letters. A
                       logo has no baseline worth speaking of, so it centres. -->
                  <span class="flex items-baseline gap-1.5">
                    {#if act.logoUrl}
                      <img
                        ...
                        title={act.name}
                        loading="lazy"
                        class="h-4 w-auto self-center object-contain"
                        style="opacity: 0.85"
                      />
                    {:else}
                      <span
                        class="truncate text-xs font-semibold"
                        style="color: var(--color-text-muted); opacity: {act.isSelf ? 1 : 0.75}"
                      >
                        {act.name}
                      </span>
                    {/if}
              ...
              <div class="relative sm:hidden">
                <button
                  onclick={(e) => {
                    ...
                    document
                      .querySelectorAll('[data-show-menu]')
                  ...
                  {#if !tour.soldOut}
                    <button
                      onclick={(e) => {
                        ...
                        e.currentTarget.closest('[data-show-menu]')?.classList.add('hidden');
                      }}
