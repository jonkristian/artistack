<script lang="ts">
  ...
  import type { Block, ShowsBlockConfig } from '$lib/server/schema';
...
<div class="space-y-3">
  <div>
    <label for="show-heading-{block.id}" class={labelClass}>Section Heading</label>
    <input
      id="show-heading-{block.id}"
      type="text"
  ...
  >
    ...
    <input
      type="checkbox"
      checked={config.showPastShows !== false}
      onchange={toggleShowPastShows}
