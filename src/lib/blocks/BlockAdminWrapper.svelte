	...
	<div class="flex items-center gap-2 px-4 py-3">
		...
		{#if editingLabel}
			<input
				...
				class="flex-1 rounded border border-gray-700 bg-gray-800 px-2 py-0.5 text-sm text-white focus:border-gray-600 focus:outline-none"
			/>
		{:else}
			<button
				onclick={() => { editingLabel = true; labelInput = block.label || def?.defaultLabel || block.type; }}
				class="flex-1 cursor-text text-left text-sm font-medium text-gray-300 hover:text-white"
			>
		...
		<div class="flex items-center gap-1">
			...
			{#if settings}
				<button
					...
					class="rounded p-1 transition-colors hover:bg-gray-800 {settingsOpen ? 'text-violet-400' : 'text-gray-600 hover:text-gray-400'}"
