<script lang="ts">
  ...
  const isValidEmail = $derived(value.length === 0 || /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value));
...
<div>
  ...
  <div class="relative">
    <input
      ...
      class="w-full rounded-lg border {showError
        ? 'border-red-500'
        : 'border-gray-600'} bg-gray-800 px-3 py-2 pr-10 text-sm text-white placeholder-gray-500 focus:border-gray-500 focus:outline-none"
    />
    {#if value.length > 0}
