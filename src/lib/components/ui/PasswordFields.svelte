  ...
  <div>
    <div class="relative">
      ...
      {#if newPassword.length > 0}
        <span
          class="absolute top-1/2 right-3 -translate-y-1/2 text-xs {meetsMinLength
            ? 'text-green-400'
            : 'text-gray-500'}"
        >
  ...
  <div>
    <div class="relative">
      <input
        ...
        class="w-full rounded-lg border {passwordsDontMatch
          ? 'border-red-500'
          : passwordsMatch
            ? 'border-green-500'
            : 'border-gray-600'} bg-gray-800 px-3 py-2 pr-10 text-sm text-white placeholder-gray-500 focus:outline-none"
      />
