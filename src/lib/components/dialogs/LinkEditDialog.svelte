	...
	{#if link}
		<div class="p-6">
			...
			<div class="space-y-4">
				...
				{#if isBandcamp && hasEmbed}
					<div class="border-t border-gray-700 pt-4">
						...
						<label class="mb-4 flex cursor-pointer items-center justify-between">
							...
							>
								<span class="absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white transition-transform {embedEnabled ? 'translate-x-5' : ''}"></span>
							</button>
						</label>
						...
						{#if embedEnabled}
							<div class="mb-4">
								...
								<div class="flex gap-2">
									<button type="button" onclick={() => (embedSize = 'small')} class="flex-1 rounded-lg px-3 py-2 text-sm transition-colors {embedSize === 'small' ? 'bg-violet-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}">Slim bar</button>
									<button type="button" onclick={() => (embedSize = 'large')} class="flex-1 rounded-lg px-3 py-2 text-sm transition-colors {embedSize === 'large' ? 'bg-violet-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}">With artwork</button>
								</div>
							</div>
							...
							<div class="mb-4">
								...
								<div class="flex gap-2">
									<button type="button" onclick={() => (bgColor = 'ffffff')} class="flex-1 rounded-lg px-3 py-2 text-sm transition-colors {bgColor === 'ffffff' ? 'bg-violet-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}">Light</button>
									<button type="button" onclick={() => (bgColor = '333333')} class="flex-1 rounded-lg px-3 py-2 text-sm transition-colors {bgColor !== 'ffffff' ? 'bg-violet-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}">Dark</button>
								</div>
							</div>
							...
							<div class="mb-4">
								...
								<div class="flex items-center gap-2">
									<input id="link-color" type="color" bind:value={linkColor} class="h-8 w-8 cursor-pointer rounded border-0 bg-transparent" />
									<input type="text" bind:value={linkColor} class="w-full rounded border border-gray-700 bg-gray-800 px-2 py-1 text-xs text-white" />
								</div>
							</div>
							...
							{#if embedSize === 'large'}
								<div class="mb-4">
									...
									<div class="flex gap-2">
										{#each ['small', 'large', 'none'] as artworkSize}
											<button type="button" onclick={() => (artwork = artworkSize as 'small' | 'large' | 'none')} class="flex-1 rounded-lg px-3 py-2 text-sm capitalize transition-colors {artwork === artworkSize ? 'bg-violet-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}">{artworkSize}</button>
										{/each}
									</div>
								</div>
								...
								<label class="flex cursor-pointer items-center justify-between">
									...
									<button type="button" role="switch" aria-checked={tracklist} onclick={() => (tracklist = !tracklist)} class="relative h-6 w-11 rounded-full transition-colors {tracklist ? 'bg-violet-600' : 'bg-gray-700'}">
										<span class="absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white transition-transform {tracklist ? 'translate-x-5' : ''}"></span>
									</button>
								</label>
							{/if}
						{/if}
				...
				{#if isSpotify && hasEmbed}
					<div class="border-t border-gray-700 pt-4">
						...
						<label class="mb-4 flex cursor-pointer items-center justify-between">
							<span class="text-sm text-gray-400">Show as player</span>
							<button type="button" role="switch" aria-checked={embedEnabled} onclick={() => (embedEnabled = !embedEnabled)} class="relative h-6 w-11 rounded-full transition-colors {embedEnabled ? 'bg-violet-600' : 'bg-gray-700'}">
								<span class="absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white transition-transform {embedEnabled ? 'translate-x-5' : ''}"></span>
							</button>
						</label>
						...
						{#if embedEnabled}
							<div class="mb-4">
								<span class="mb-2 block text-sm text-gray-400">Player theme</span>
								<div class="flex gap-2">
									<button type="button" onclick={() => (spotifyTheme = 'dark')} class="flex-1 rounded-lg px-3 py-2 text-sm transition-colors {spotifyTheme === 'dark' ? 'bg-violet-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}">Dark</button>
									<button type="button" onclick={() => (spotifyTheme = 'light')} class="flex-1 rounded-lg px-3 py-2 text-sm transition-colors {spotifyTheme === 'light' ? 'bg-violet-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}">Light</button>
								</div>
							</div>

							<label class="flex cursor-pointer items-center justify-between">
								<span class="text-sm text-gray-400">Compact player</span>
								<button type="button" role="switch" aria-checked={spotifyCompact} onclick={() => (spotifyCompact = !spotifyCompact)} class="relative h-6 w-11 rounded-full transition-colors {spotifyCompact ? 'bg-violet-600' : 'bg-gray-700'}">
									<span class="absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white transition-transform {spotifyCompact ? 'translate-x-5' : ''}"></span>
								</button>
							</label>
						{/if}
					</div>
			...
			<div class="mt-6 flex items-center justify-between border-t border-gray-700 pt-4">
				<button onclick={handleDelete} class="text-sm text-red-400 hover:text-red-300">Delete link</button>
				<div class="flex gap-2">
					<button onclick={handleClose} class="rounded-lg bg-gray-800 px-4 py-2 text-sm font-medium text-gray-300 hover:bg-gray-700">Cancel</button>
					<button onclick={handleSave} class="rounded-lg bg-violet-600 px-4 py-2 text-sm font-medium text-white hover:bg-violet-500">Apply</button>
				</div>
			</div>
		</div>
	{/if}
