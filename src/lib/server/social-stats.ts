 ...
 */
export async function fetchSpotifyArtistStats(
...
export async function detectSpotifyArtistFromLinks(): Promise<string | null> {
  const spotifyLinks = await db.select().from(links).where(eq(links.platform, 'spotify'));
...
export async function detectYouTubeChannelFromLinks(apiKey?: string): Promise<string | null> {
  const youtubeLinks = await db.select().from(links).where(eq(links.platform, 'youtube'));
 ...
 */
export async function updateIntegrationCache(provider: string, cachedData: unknown): Promise<void> {
...
export async function getDetectedPlatformIds(): Promise<{
  ...
  youtube: {
    ...
    rawId: string | null;
    type: 'channel' | 'handle' | 'custom' | null;
  };
}> {
