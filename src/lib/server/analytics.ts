...
export interface LinkClickStats {
	totalClicks: number;
	clicksByLink: { linkId: number; label: string | null; platform: string; url: string; count: number }[];
...
export async function getOverviewStats(): Promise<OverviewStats> {
	...
	if (topLinks.length > 0) {
		const [linkData] = await db.select().from(links).where(eq(links.id, topLinks[0].linkId)).limit(1);
		if (linkData) {
			topLink = {
...
export async function getPageViewStats(days: number = 30): Promise<PageViewStats> {
	...
	return {
		...
		viewsByReferrer: viewsByReferrer.map((r) => ({ referrer: r.referrer ?? 'direct', count: r.count })),
...
export async function getLinkClickStats(days: number = 30): Promise<LinkClickStats> {
	...
	const linkDetails = linkIds.length > 0
		? await db.select().from(links).where(sql`${links.id} IN (${sql.join(linkIds, sql`, `)})`)
		: [];
	...
	return {
		...
		clicksByReferrer: clicksByReferrer.map((r) => ({ referrer: r.referrer ?? 'direct', count: r.count })),
		clicksByCountry: clicksByCountry.map((r) => ({ country: r.country ?? 'Unknown', count: r.count }))
	};
}
...
export async function getComparisonStats(
	days: number = 30
): Promise<{ currentViews: number; previousViews: number; currentClicks: number; previousClicks: number }> {
	const now = new Date();
	const currentStart = new Date(now.getTime() - days * 24 * 60 * 60 * 1000);
	const previousStart = new Date(now.getTime() - days * 2 * 24 * 60 * 60 * 1000);
	...
	const [previousViewsResult] = await db
		.select({ count: count() })
		.from(pageViews)
		.where(and(gte(pageViews.createdAt, previousStart), sql`${pageViews.createdAt} < ${currentStart.getTime()}`));
	...
	const [previousClicksResult] = await db
		.select({ count: count() })
		.from(linkClicks)
		.where(and(gte(linkClicks.createdAt, previousStart), sql`${linkClicks.createdAt} < ${currentStart.getTime()}`));
