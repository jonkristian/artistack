<script lang="ts">
  ...
  const passwordsValid = $derived(password.length >= 8 && password === confirmPassword);

  const newPasswordsValid = $derived(newPassword.length >= 8 && newPassword === confirmNewPassword);
    ...
    <div class="relative rounded-3xl bg-gray-900 p-8 shadow-2xl ring-1 ring-white/10">
      ...
      {#if isSetup}
        ...
        >
          ...
          <EmailField value={email} onchange={(v) => (email = v)} name="email" required />
          ...
          <button
            ...
            class="w-full cursor-pointer rounded-lg px-3 py-2.5 text-sm font-medium text-white transition-colors disabled:cursor-not-allowed disabled:opacity-50 {passwordsValid
        ...
        <div class="space-y-4">
          ...
          {:else if recoveryStep === 'otp'}
            ...
            <p class="text-sm text-gray-400">
              If an account exists for <span class="text-white">{recoveryEmail}</span>, we've sent a
              6-digit code.
            </p>
