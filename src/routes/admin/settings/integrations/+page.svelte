<script lang="ts">
  ...
  import { ToggleSwitch, DateTimePicker } from '$lib/components/ui';
  ...
  async function togglePages() {
    ...
    await updateSettings({ pagesEnabled });
    await invalidateAll();
  }
...
<div class="max-w-2xl space-y-6">
  ...
  <section class="rounded-xl border border-gray-800 bg-gray-900 p-5">
    <div class="mb-4 flex items-center justify-between">
      <div class="flex items-center gap-3">
        <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-indigo-600/20">
          <svg
    ...
    <div class="space-y-4">
      ...
      {#if discordEnabled}
        ...
        >
          ...
          <div>
            <span class="mb-1 block text-xs text-gray-500">Time</span>
            <DateTimePicker
              mode="time"
              compact
              value={discordScheduleTime}
              locale={data.settings?.locale || 'nb-NO'}
              onchange={(v) => (discordScheduleTime = v)}
            />
          </div>
        </div>
      {/if}
  ...
  <div class="pt-4">
    ...
    <section class="rounded-xl border border-gray-800 bg-gray-900 p-5">
      ...
      <p class="mt-3 text-xs text-gray-500">
        When enabled, you can create a downloadable press kit with high-res images and bio from the
        Media page.
      </p>
    </section>
    ...
    <section class="mt-6 rounded-xl border border-gray-800 bg-gray-900 p-5">
      ...
      {#if !pixelsEnabled}
        <p class="mt-3 text-xs text-gray-500">
          Adds Meta and TikTok pixels to your public pages, and reports a conversion from the server
          when someone opens a streaming link — the half an ad blocker can't drop.
        </p>
      {:else}
        ...
        <div class="mt-4 rounded-lg border border-amber-700/50 bg-amber-950/40 p-3 text-sm">
          ...
          <p class="mt-1 text-amber-200/70">
            In the EEA that needs the visitor's consent <em>before</em> the script loads. Artistack has
            no consent banner yet, so leave this off on a public site until it does, or add one.
          </p>
        </div>
        ...
        <div class="mt-4">
          ...
          <p class="mt-1 text-xs text-gray-500">
            Optional, and only used server-side. Without it the browser pixel still works; with it,
            a click through to a streaming service is reported even when the pixel is blocked.
          </p>
        </div>
    ...
    <section class="mt-6 rounded-xl border border-gray-800 bg-gray-900 p-5">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="font-semibold text-white">Shows</h3>
            <p class="text-xs text-gray-500">Your gigs, kept in one place</p>
          </div>
        </div>
        <ToggleSwitch
          checked={showsEnabled}
          label="Toggle shows"
          onchange={toggleShows}
          size="md"
          hideLabel
        />
      </div>
      ...
      {#if !showsEnabled}
        <p class="mt-3 text-xs text-gray-500">
          Adds a Shows section for your tour dates. They belong to the site rather than to one
          block, so a Tour Dates block on any page lists the same shows and removing that block
          doesn't remove the tour.
        </p>
      {/if}
    </section>
    ...
    <section class="mt-6 rounded-xl border border-gray-800 bg-gray-900 p-5">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="font-semibold text-white">Extra pages</h3>
            <p class="text-xs text-gray-500">An about or a contact, at its own address</p>
          </div>
        </div>
        <ToggleSwitch
          checked={pagesEnabled}
          label="Toggle extra pages"
      ...
      {#if !pagesEnabled}
        <p class="mt-3 text-xs text-gray-500">
          Your front page, releases and shop already have their own sections. Turn this on only if
          you want ordinary pages besides those — each one gets an address and the same blocks the
          front page is built from.
        </p>
      {/if}
    </section>
      ...
      {:else}
        ...
        <div class="mt-4 flex items-center justify-between border-t border-gray-800 pt-4">
          <div>
            ...
            <p class="text-xs text-gray-500">
              POST a queued clip to your webhook as its slot comes due — video link, poster, preview
              link and post sheet as JSON. Whatever receives it (n8n, a script, your own service)
              owns the platform credentials.
            </p>
          </div>
          <ToggleSwitch
      ...
      {#if clipsEnabled && publishEnabled}
        ...
        <div class="mt-4 space-y-4 border-t border-gray-800 pt-4">
          ...
          <p class="text-xs text-gray-600">
            ...
            header (HMAC-SHA256 of the body), so the receiver can verify the call came from here. It also
            authenticates the callback the receiver makes to report where a clip was published — without
            a secret set, that callback is rejected.
          </p>
        </div>
      {/if}
    </section>
