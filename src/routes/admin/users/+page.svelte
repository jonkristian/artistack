<script lang="ts">
	...
	import { addUser, updateUser, deleteUser, updateOwnProfile, changePassword, resetPassword } from './users.remote.js';
	...
	<div class="max-w-2xl space-y-6">
				...
				<div class="space-y-3">
					...
					<div class="flex items-center gap-2">
						{#if currentUser?.image}
							<img src={currentUser.image} alt="" class="h-10 w-10 shrink-0 rounded-full object-cover" />
						{:else}
							<div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gray-700 text-sm font-medium text-gray-300">
					...
					<div class="border-t border-gray-700 pt-3">
						<span class="mb-2 block text-xs text-gray-500">Change password (leave empty to keep current)</span>
						<div class="space-y-2">
							<input
								type="password"
			...
			{:else}
				<div class="flex items-center justify-between">
					<div class="flex items-center gap-3">
						...
						{:else}
							<div class="flex h-10 w-10 items-center justify-center rounded-full bg-gray-700 text-sm font-medium text-gray-300">
								{currentUser?.name?.charAt(0).toUpperCase() || '?'}
							</div>
						{/if}
						<div>
							<div class="flex items-center gap-2">
								...
								<span class="rounded px-1.5 py-0.5 text-xs {getRoleBadgeClass(currentUser?.role ?? null)}">
		...
		<SectionCard>
			<div class="mb-4 flex items-center justify-between">
				<h2 class="text-sm font-medium uppercase tracking-wider text-gray-400">Team Members</h2>
				{#if isAdmin}
					<button
					...
					{...addForm.enhance(async ({ submit }) => {
						try {
							...
							if (!issues || issues.length === 0) {
								showAddForm = false;
								addForm.fields.set({ name: '', email: '', password: '', confirmPassword: '', role: 'editor' });
								await invalidateAll();
							}
						} catch (e) {
			...
			<div class="space-y-2">
				{#each data.users.filter(u => u.id !== currentUser?.id) as u (u.id)}
					<div class="group rounded-lg bg-gray-800/50 px-4 py-3 transition-colors hover:bg-gray-800">
						{#if editingUserId === u.id && isAdmin}
							...
							<div class="space-y-3">
								...
								<div class="flex items-center gap-2">
									...
									{:else}
										<div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gray-700 text-sm font-medium text-gray-300">
								...
								<div class="border-t border-gray-700 pt-3">
									<span class="mb-2 block text-xs text-gray-500">New password (leave empty to keep current)</span>
									<PasswordFields
										newPassword={resetNewPassword}
										confirmPassword={resetConfirmPassword}
						...
						{:else}
							...
							<div class="flex items-center justify-between">
								<div class="flex items-center gap-3">
									...
									{:else}
										<div class="flex h-9 w-9 items-center justify-center rounded-full bg-gray-700 text-sm font-medium text-gray-300">
											{u.name.charAt(0).toUpperCase()}
										</div>
									{/if}
									<div>
		...
		<div class="rounded-lg border border-gray-800 bg-gray-900/50 p-4">
			...
			<ul class="space-y-1 text-xs text-gray-500">
				...
				<li><span class="text-gray-400">Editor:</span> Can edit links, tour dates, and profile content</li>
			</ul>
		</div>
	</div>
</div>
