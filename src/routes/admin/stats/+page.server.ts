...
export const load: PageServerLoad = async () => {
  ...
  const viewsChange =
    comparison.previousViews > 0
      ? Math.round(
          ((comparison.currentViews - comparison.previousViews) / comparison.previousViews) * 100
        )
  ...
  const clicksChange =
    comparison.previousClicks > 0
      ? Math.round(
          ((comparison.currentClicks - comparison.previousClicks) / comparison.previousClicks) * 100
        )
