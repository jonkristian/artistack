		...
		<section class="rounded-xl border border-gray-800 bg-gray-900 p-5">
			<div class="mb-4 flex items-center justify-between">
				<div class="flex items-center gap-3">
					...
					<div>
						<h2 class="font-semibold text-white">Spotify for Artists</h2>
						<p class="text-xs text-gray-500">Track followers and popularity stats</p>
					</div>
				</div>
				{#if data.socialStats.spotify}
					<span class="rounded-full bg-green-900/50 px-2.5 py-1 text-xs text-green-400">Connected</span>
				{/if}
			</div>
			...
			{#if data.socialStats.spotify}
				<div class="mb-4 grid grid-cols-3 gap-4 rounded-lg bg-gray-800/50 p-4">
					<div>
						...
						<div class="text-lg font-semibold text-white">{formatNumber(data.socialStats.spotify.followers)}</div>
					</div>
					<div>
						<div class="text-xs text-gray-500">Popularity</div>
						<div class="text-lg font-semibold text-white">{data.socialStats.spotify.popularity}/100</div>
					</div>
					<div>
						<div class="text-xs text-gray-500">Last Updated</div>
						<div class="text-sm text-gray-300">{new Date(data.socialStats.spotify.lastUpdated).toLocaleDateString()}</div>
					</div>
				</div>
			{/if}
			...
			{:else}
				<div class="mb-4 rounded-lg border border-yellow-800/50 bg-yellow-900/20 p-3">
					...
					<p class="mt-1 text-xs text-gray-400">Add a Spotify artist link to your page first, or it will be detected automatically.</p>
				</div>
			{/if}
			...
			<div class="space-y-3">
				<div class="grid grid-cols-2 gap-3">
					...
					<div>
						<label for="spotifyClientSecret" class="mb-1 block text-xs text-gray-500">Client Secret</label>
						<input
				...
				<div class="flex items-center gap-3">
					<button
						...
						disabled={savingSpotify || !spotifyDetectedId || !spotifyClientId || !spotifyClientSecret}
				...
				<p class="text-xs text-gray-600">
					Get credentials from <a href="https://developer.spotify.com/dashboard" target="_blank" rel="noopener" class="text-green-400 hover:underline">Spotify Developer Dashboard</a>
				</p>
			</div>
		</section>
		...
		<section class="rounded-xl border border-gray-800 bg-gray-900 p-5">
			<div class="mb-4 flex items-center justify-between">
				...
				{#if data.googleConfig?.apiKey}
					<span class="rounded-full bg-blue-900/50 px-2.5 py-1 text-xs text-blue-400">Connected</span>
				{/if}
			</div>
			...
			{#if data.socialStats.youtube && googleYoutubeEnabled}
				<div class="mb-4 grid grid-cols-3 gap-4 rounded-lg bg-gray-800/50 p-4">
					<div>
						...
						<div class="text-lg font-semibold text-white">{formatNumber(data.socialStats.youtube.subscriberCount)}</div>
					</div>
					<div>
						<div class="text-xs text-gray-500">Total Views</div>
						<div class="text-lg font-semibold text-white">{formatNumber(data.socialStats.youtube.viewCount)}</div>
					</div>
					<div>
						<div class="text-xs text-gray-500">Videos</div>
						<div class="text-lg font-semibold text-white">{data.socialStats.youtube.videoCount}</div>
					</div>
				</div>
			{/if}
			...
			<div class="space-y-4">
				...
				<div class="space-y-3 rounded-lg bg-gray-800/50 p-3">
					<label class="flex items-center justify-between">
						...
						<button
							...
							class="relative h-6 w-11 rounded-full transition-colors {googlePlacesEnabled ? 'bg-blue-600' : 'bg-gray-600'}"
						>
							<span class="absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white transition-transform {googlePlacesEnabled ? 'translate-x-5' : ''}"></span>
						</button>
					</label>
					...
					<label class="flex items-center justify-between">
						<div>
							...
							{:else if googleYoutubeEnabled && !youtubeDetectedId}
								<p class="mt-1 text-xs text-yellow-400">Add a YouTube channel link to enable stats</p>
							{/if}
						</div>
						<button
							type="button"
							...
							class="relative h-6 w-11 rounded-full transition-colors {googleYoutubeEnabled ? 'bg-blue-600' : 'bg-gray-600'}"
						>
							<span class="absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white transition-transform {googleYoutubeEnabled ? 'translate-x-5' : ''}"></span>
						</button>
					</label>
				</div>
				...
				<p class="text-xs text-gray-600">
					Get API key from <a href="https://console.cloud.google.com/apis/credentials" target="_blank" rel="noopener" class="text-blue-400 hover:underline">Google Cloud Console</a>. Enable Places API and/or YouTube Data API v3.
				</p>
			</div>
		</section>
		...
		<section class="rounded-xl border border-gray-800 bg-gray-900 p-5">
			<div class="mb-4 flex items-center justify-between">
				...
				{#if discordEnabled && discordWebhookUrl}
					<span class="rounded-full bg-indigo-900/50 px-2.5 py-1 text-xs text-indigo-400">Active</span>
				{/if}
			</div>
			...
			<div class="space-y-4">
				...
				<label class="flex items-center justify-between">
					...
					<button
						...
						class="relative h-6 w-11 rounded-full transition-colors {discordEnabled ? 'bg-indigo-600' : 'bg-gray-600'}"
					>
						<span class="absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white transition-transform {discordEnabled ? 'translate-x-5' : ''}"></span>
					</button>
				</label>
					...
					<div class="grid grid-cols-3 gap-3 rounded-lg bg-gray-800/50 p-3">
						...
						<div>
							<label for="discordScheduleDay" class="mb-1 block text-xs text-gray-500">
								{discordSchedule === 'weekly' ? 'Day' : discordSchedule === 'monthly' ? 'Day' : 'N/A'}
							</label>
